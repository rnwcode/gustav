...
CandidatePool collectCandidates({
  ...
  required List<Priority> priorities,       // aus WeeklyContext.priorities
  required DateTime periodEnd,
  required Map<NeedDimension, int> needCoverageLastPeriod,
  required CandidateConfig config,          // needTargets aus bedarf_ziel
})
```

`CandidatePool` trägt zwei Listen:

- `skills: List<SkillFocus>` — `{skillId, levels, priority (0–3),
...
- `needs: List<NeedFocus>` — `{dimension, gap (>0)}`. Nur Dimensionen mit
...
1. **Fällige Auffrischungen (a)**: jeder Eintrag in `skillStates` mit
   ...
   `overdueDays = periodEnd.difference(dueAt).inDays` (≥ 0, da nur fällige
   Skills überhaupt betrachtet werden).
...
1. **Fällige Auffrischung.**
   Eingabe: `skillStates = {'rueckruf': SkillState(status: generalizing,
   levels: {1,3,2}, dueAt: 2026-03-10)}`, `catalog = [rueckruf]`,
   `priorities = []`, `periodEnd = 2026-03-12`.
   Ausgabe: `skills = [SkillFocus(skillId: 'rueckruf', levels: {1,3,2},
   priority: 0, overdueDays: 2, isNewSkill: false)]`.

2. **Priorität hebt einen nicht fälligen Skill ins Rennen.**
   Eingabe: `skillStates = {'leash': SkillState(status: building,
   levels: {0,1,0}, dueAt: 2026-03-20)}` (nach `periodEnd`),
   `priorities = [Priority('leash', weight: 3)]`, `periodEnd =
   2026-03-12`.
   Ausgabe: `skills = [SkillFocus(skillId: 'leash', levels: {0,1,0},
...
3. **Fällig und priorisiert verschmelzen zu einem Eintrag.**
   ...
   `priorities = [Priority('rueckruf', weight: 2)]`, `periodEnd =
   2026-03-12`.
   Ausgabe: genau ein `SkillFocus(skillId: 'rueckruf', levels: {1,3,2},
   priority: 2, overdueDays: 2, isNewSkill: false)` — nicht zwei Einträge.

4. **Neuer Skill nur mit erfüllter Voraussetzung und Alter.**
   Eingabe: `catalog = [Skill('recall', prerequisites: ['name-focus'],
   minAgeWeeks: 9, …)]`, `dogAgeWeeks = 12`.
   4a. `skillStates = {'name-focus': SkillState(status: generalizing, …)}`
   → `recall` erscheint als `SkillFocus(isNewSkill: true, levels: {0,0,0},
   priority: 0, overdueDays: 0)`.
   4b. `skillStates = {'name-focus': SkillState(status: building, …)}`
...
5. **Bedarfslücke.**
   ...
   Ausgabe: `needs = [NeedFocus(scent, gap: 3), NeedFocus(social, gap:
   3)]` — beide Lücken werden erkannt, auch die durch komplett fehlende
   Erfassung.
